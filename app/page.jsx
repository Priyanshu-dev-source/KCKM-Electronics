...
export default function Home() {
  ...
  //   setScrollVal(currentDelta);
  //   if (previousDeltaRef.current === currentDelta) return;

  //   previousDeltaRef.current = currentDelta;

  //   setScaleDeltaValue((prev) => {
  //     const newVal =
  //       currentDelta < 0 ? Math.max(prev - 0.02, 1) : Math.min(prev + 0.05, 2);
  //     return newVal;
  //   });

  //   setLeftDeltaValue((prev) => {
  //     const newVal = prev + currentDelta * -2.3;
  ...
  //         // if (isImageScrolled) return;
  //         // window.scroll({ top: window.screenTop + 600, behavior: "smooth" });
  //         // setIsImageScrolled(true);
    ...
    <>
      ...
      >
        <div className="fixed top-[0px] h-[70px] w-dvw backdrop-blur-[5px] z-[4]">
          ...
          <div
            className={`fixed top-[70px] left-0 right-0 bg-black/90 text-white z-[45] rounded-b-xl transition-transform duration-300 ease-in-out ${
        ...
        <div
          className={` ${headingDisplay} h-[550px] w-[800px] z-[11] flex items-center justify-center flex-col`}
        >
          <h1
            style={{
